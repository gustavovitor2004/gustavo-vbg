  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < BANDS; i++) {
        ...
        // Each band rotates at slightly different speed — outer slower
        const speed = 1 + (1 - pct) * 1.8;
        const offset = (i % 2 === 0 ? 1 : -1) * t * speed + i * 0.55;
        // Arc length decreases at edges
        const arcLen = Math.PI * (1.2 + pct * 0.6);
        const alpha =
          pct < 0.15
            ? 0.22
            : pct > 0.75
            ? 0.18 * (1 - (pct - 0.75) / 0.25)
            : 0.2 - pct * 0.08;

        // Color: bright blue near center → grey-blue at edge
        const bVal = Math.floor(180 - pct * 80);
        const gVal = Math.floor(110 - pct * 60);

        ctx.beginPath();
        ctx.arc(0, 0, r, offset, offset + arcLen);
        ctx.strokeStyle = `rgba(55,${gVal},${bVal},${alpha})`;
      ...
      for (let i = 0; i < starCount; i++) {
        ...
        const sa = (0.2 + (i % 11) * 0.065) * twinkle;
      ...
      // 7 — Dark vignette — pushes galaxy focus to center
      const vignette = ctx.createRadialGradient(
        cx, cy, R * 0.35,
        cx, cy, Math.max(width, height) * 0.82
      );
